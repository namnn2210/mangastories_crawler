...
def new_manga_builder(manga_obj_dict, slug_format=False, publish=True):
    ...
    manga_dict = {
        'name': manga_obj_dict.get('name', ''),
        'slug': slug,
        'alt_name': manga_obj_dict.get('alternative_name', ''),
        'original': manga_obj_dict.get('source_site', ''),
        'original_id': manga_obj_dict.get('original_id', ''),
        'thumb': thumb_path,
        'manga_type_id': manga_type,
        'finished': finished,
        'status': status,
        'published': manga_obj_dict.get('published', ''),
        'description': manga_obj_dict.get('description', ''),
        'manga_authors': manga_obj_dict.get('author', ''),
        'manga_genres': manga_obj_dict.get('genre', ''),
...
def new_push_chapter_to_db(db, processed_chapter_dict, bucket, manga_id, manga_slug, upload=True, error=None):
    ...
    if chapter_count == 0:
        try:
            ...
            chapter_query = db.query(NewMangaChapters).filter(NewMangaChapters.manga_id == manga_id,
                                                              NewMangaChapters.chapter_code == float(
                                                                  manga_chapter_obj.chapter_code))
...
def new_process_push_to_db(mode='crawl', type='manga', list_update_original_id=None,
                           source_site=MangaSourceEnum.MANGASEE.value, upload=True, count=None, slug_format=False,
        ...
        if type == 'chapter' or type == 'all':
            ...
            if existed_manga is not None:
                ...
                if not bulk:
                    ...
                    new_push_chapter_to_db_bulk(db, list_chapter_dict, bucket, existed_manga.id, existed_manga.slug,
                                                upload, tx_manga_errors)


def process_push_to_db(mode='crawl', type='manga', list_update_original_id=None,
                       source_site=MangaSourceEnum.MANGASEE.value, upload=True, insert=True, count=None,
                       slug_format=False, publish=True, bulk=False):
    # Connect DB
