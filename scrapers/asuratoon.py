...
from utils.crawler_util import get_soup, format_leading_img_count, format_leading_part, process_chapter_ordinal, process_insert_bucket_mapping, new_process_push_to_db, process_push_to_db
...
class AsuratoonCrawler(Crawler):
    ...
    def crawl(self, original_ids=None):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            futures = [executor.submit(self.extract_manga_info, manga_url, mongo_collection,
                                       tx_manga_bucket_mapping, tx_manga_errors) for manga_url in list_manga_urls]
    ...
    def get_all_manga_urls(self):
        ...
        while current_url:
            ...
            list_mangas, current_url = self.process_page_urls(
    ...
    def process_page_urls(self, url, base_url):
        ...
        next_page_link = page_soup.find(
            'div', {'class': 'hpage'}).find('a', {'class': 'r'})
        if next_page_link:
    ...
    def extract_manga_info(self, manga_url, mongo_collection, tx_manga_bucket_mapping, tx_manga_errors):
        try:
            ...
            manga_status_div = manga_soup.find_all(
                'div', {'class': 'imptdt'})[0]
            if manga_status_div:
            ...
            if len(list_genres_div) > 1:
                ...
                list_genres = None
            list_processed_detail, list_processed_genres = self.process_detail(
                list_details, list_genres)

            list_chapters = manga_soup.find(
                'ul', {'class': 'clstyle'}).find_all('li')
            manga_count_chapters = len(list_chapters)

            list_chapters_info = []
            non_chapter_number_count = 0
            for chapter in list_chapters:
                chapter_info_dict, non_chapter_number = self.extract_chapter_info(
                    chapter=chapter, manga_slug=manga_slug, non_chapter_number_count=non_chapter_number_count, bucket=bucket)
            ...
            description = manga_soup.find(
                'div', {'itemprop': 'description'}).find('p')
            if description is not None:
                final_dict['description'] = description.text + \
        ...
        for detail in list_details:
            ...
            for info in list_detail_info:
                ...
                if b_tag == 'Posted On':
                    ...
                    list_processed_detail.append({'published': value.replace(
                        '\r', '').replace('\t', '').replace('\n', '')})

                elif b_tag == 'Author':
                    value = info.find('span').text
                    list_processed_detail.append({'author': value.replace(
                        '\r', '').replace('\t', '').replace('\n', '')})
    ...
    def extract_chapter_info(self, chapter, manga_slug, non_chapter_number_count, bucket):
        chapter_url = chapter.find(
            'div', {'class': 'eph-num'}).find('a')['href']
        ...
        for index, image in enumerate(list_images):
            ...
            if chapter_source_match:
                ...
                img_url = original_url.replace(
                    chapter_source_match.group(), '')
        ...
        chapter_info_dict = {
            ...
            'resources_storage': 'asuratoon.com',
    ...
    def update_manga(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            futures = [executor.submit(
                self.extract_manga_info, manga_url, mongo_collection) for manga_url in list_manga_urls]
    ...
    def push_to_db(self, mode='crawl', type='manga', list_update_original_id=None, upload=False, count=None, new=True,
        ...
        if new:
            ...
            process_push_to_db(
                ...
                source_site=MangaSourceEnum.ASURATOON.value, insert=True, upload=upload, slug_format=slug_format,
