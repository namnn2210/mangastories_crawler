...
from utils.crawler_util import get_soup, format_leading_img_count, format_leading_part, process_chapter_ordinal, \
    new_process_push_to_db, process_insert_bucket_mapping
...
class ManhuaplusCrawler(Crawler):
    ...
    def extract_manga_info(self, manga_url, mongo_collection, tx_manga_bucket_mapping):
        ...
        manga_name = manga_soup.find('div', {'class': 'post-title'}).text.replace('\n','').replace('\t','')
        ...
        for chapter in list_chapters:
            ...
            chapter_info_dict = self.extract_chapter_info(chapter=chapter, str_chapter_num=str_chapter_num,
                                                          manga_slug=manga_slug, bucket=bucket)
    ...
    def push_to_db(self, mode='crawl', type='manga', list_update_original_id=None, upload=False, count=None):
        new_process_push_to_db(mode=mode, type=type, list_update_original_id=list_update_original_id,
                               source_site=MangaSourceEnum.MANHUAPLUS.value, upload=upload, count=count)
